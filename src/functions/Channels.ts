...
export class Channel<T extends keyof PrefixTable> {
    ...
    private writeState = () => {
        ...
        if (stateString === localStorage.getItem(this.stateChannel)) {
            return
        }
        localStorage.setItem(this.stateChannel, stateString)
    }
    private startWrite = () => {
        if (this.state) this.stopWrite = this.writeState
    }
    ...
    private update = (val: string | null) => {
        ...
        if (val) {
            this.set(JSON.parse(val))
        }
        this.startWrite()
    }
    private storageListener = (e: StorageEvent) => {
        if (
            e.key !== this.stateChannel ||
    ...
    set = (newState: PrefixTable[T]) => {
        ...
        if (Array.isArray(this.state) && Array.isArray(newState)) {
            ...
            for (const key in this.state) {
                !(key in newState) && delete this.state[key]
            }
...
const appInfo = {
    name: hash.get("name") || undefined,
    logo: hash.get("logo") || undefined,
}
...
const { state, destructor } = new Channel(chPrefix, instance, {
    origin,
    session,
})
...
function getChannels<T extends "connectorState:" | "sharedState:">(prefix: T) {
    ...
    const instantiate = async (name: string) => {
        ...
        if (prefix === chPrefix && !(await heartbeat(name))) {
            close(name)
            return null
        }
    ...
    const storageListener = () => {
        ...
        for (const channel of [...runningChannels, ...storageChannels]) {
            if (
                runningChannels.includes(channel) &&
                storageChannels.includes(channel)
            ) {
                continue
            } else if (
                storageChannels.includes(channel) &&
                ...
                instantiate(channel)
            } else if (runningChannels.includes(channel)) {
                close(channel)
            }
        }
    }
...
async function heartbeat(instanceName: string, timeout?: number) {
    ...
    const promise = new Promise((resolve) => {
        ...
        const heartbeatReturn = (result: boolean) => {
            ...
            if (!result) {
                localStorage.removeItem(chPrefix + instanceName)
            }
        ...
        const cleanupTimeout = setTimeout(
            () => heartbeatReturn(false),
            Math.max(5000, timeout || 0)
        )
        if (timeout) {
...
function cleanHeartbeats() {
    for (const key in localStorage) {
        if (key.slice(0, heartbeatPrefix.length) !== heartbeatPrefix) {
            continue
        }
        const relatedChannel = chPrefix + key.slice(heartbeatPrefix.length)
        if (localStorage.getItem(relatedChannel)) {
            continue
        }
...
export function filterChannels(filter: object, object = states) {
    const filterFunction = ([key, state]: [string, any]) =>
...
function globalStorageListener(e: StorageEvent) {
    ...
    if (
        e.key?.slice(0, partialKey.length) === partialKey &&
        e.newValue === ""
    ) {
